...
dt_all <- fread("app-data/uber-data.csv") %>%
...
server <- function(input, output, session) {
  ...
  updateSelectInput(session, "base",
                    label = "Select base to filter by:",
                    choices = c("All", unique(dt_all[["Base"]])),
                    selected = "All"
  )
  
  updateSelectInput(session, "borough",
                    label = "Select borough to filter by:",
                    choices = c("All", unique(dt_all[["Borough"]]), "Greater New York Area"),
                    selected = "All"
  )
  ...
  output$monthHourChart <- renderPlot({
  ...
  output$monthHourHeat <- renderPlot({
    dt_month_hours <- dt_filtered() %>%
      .[, .(Trips = sum(.N)), by = c("Formatted.Month", "Formatted.Hour")]
  ...
  output$monthDayHeat <- renderPlot({
  ...
  output$monthWdayHeat <- renderPlot({
  ...
  output$baseWdayHeat <- renderPlot({
